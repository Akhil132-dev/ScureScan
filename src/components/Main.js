...
function Main() {
  ...
  const loadBlockchain = async () => {
    ...
    scan = new web3.eth.Contract(
      ScureScan.abi,
      "0x3Dbc890A84136bf366F343890A0B7982D6f77E18"
    );
    setscan1(scan);
    setisloding(false);
  ...
  const generatSHA256hase = async () => {
    ...
    return SHA256(
      text + productname + serialnumber + rendom.toString() + date.toString()
    ).toString();
  };
  ...
  const handleScanFile = async (result) => {
    ...
    if (result) {
      ...
      console.log("im here");
      let Presult = await scan1.methods
        .verify_Product(result)
        .send({ from: account });
      sett(Presult);
  ...
  const handleScanWebCam = async (result) => {
    if (result) {
      isture1 = await scan1.methods.verify_Product(result).call();
      if (isture1.istrue) {
    ...
    <div>
      {isloding ? (
        ...
        <Container className={classes.conatiner}>
          <Card>
            {text2 ? (
              <h2 className={`  ${t ? classes.title : classes.title3}  `}>
                {t ? (
                  <p>product is Cheaked Status : Real </p>
                ) : (
                  <p>product is Cheaked Status : Fake </p>
                )}
              </h2>
            ) : (
            ...
            <p className={classes.p}>
              <strong>
                {" "}
                [ Enter Detiles] --- > [Click to Generate] ---> [Click to Image
                ] --> [Click to Scan qr code] ----> [Select Qr Code Pic ] -->
                [open] -->you will get result
              </strong>
            </p>
          ...
          <p className={classes.p}>
            {" "}
            this is a <strong>Blockchian</strong> Based Application that used{" "}
            <strong>Ethereum blockchain</strong>,{" "}
            <strong> solidity smart contract </strong> to work with ethereum,{" "}
            <strong> web3 js </strong> for connection front end to blockchain
            <strong>Qr code js </strong> for generateing Qr code of the product
            and <strong>Qr Readr js </strong> for reading Qr code of the product
            with that it also uses <strong> SHA256 hase </strong> function from
            the crypto-js for generateing product hash we can uses product
            detiles for generateing product hash for simplcity perpuse we uses
            product name , serial number and text , current data and time and a
            rendom number between 1 to 1000000000 that make it more sqcure hase
            <br />
            <h1 className={classes.p}>
              lets talk how does it works and why we need this{" "}
            </h1>
            NOw a days everything is running on internet like shoping is one of
            the most thing that people do form thire comfort but there are some
            problem with that some time we order something and we get something
            else or get a fake product that is not good for us so this aap help
            us to find out if the product is real or not this uses product
            detiles for generateing product hase and store it on the blockchain
            while creating product and after that it Generate a qr code of that
            that hase that qr code we can use to identife that product is real
            or not it Cheaked only once means you can not use it multiple time
            for multiple product once you sacn it you can not scan it again if
            you try so it show you that your product is fake means we can not
            use it multiple time on multiple product for a product we only have
            a diffrent hase that store on the blockchain and we can only read it
            once .
            <br />
            <br />
            how does it works
            ...
            <strong>
              {" "}
              [ Proudct detiles ] -----Generate Hash SHA256---> [Generate Qr
              code ] ----Store on Blockchain---> [ hash --> ture ]--Scan Qr Code
              with Qr reader--> [get code Hash] ----checked the hash into the
              blockchain---> [retrun true based on product else false] --->
              acrroding that we show result on front end{" "}
            </strong>
          </p>
        </Container>
      )}
